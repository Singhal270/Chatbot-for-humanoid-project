...
def takequiz():
        ...
        exit1 = Button(window, text="Exit",font=("Arial",14),command=khatam, width=15, height=3)
        exit1.grid(column=0,row=1,padx=218,pady=170)

        questions = [["it is first question","1","2","3","4"]]
        questions.append(["it is second question ?","1","2","3","4"])
        ...
        class Quiz:
                ...
                def Frage(self):
                        ...
                        if len(self.Fragen) > 0 and self.nummer < self.Max:
                                ...
                                clear()
                                lb = Label(window, text="you answered correctly " + str(self.right) + " questions out of " + str(self.Max) + ",thanks ",font=("Arial",14))
                ...
                def control1(self):
                        if self.lock == False:                    # here I have to add database
                                if self.Ra != self.a1:
                                        self.antw1.configure(bg="red")   # option 1 become red if false
                                else:
                                        self.antw1.configure(bg="green")  # option 1 become green if true
                                        self.right += 1
                                self.RaBtn.configure(bg="green")  # option right become green
                                self.lock = True
